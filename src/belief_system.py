...
class BeliefTracker:
    ...
    def __init__(self):
        ...
        self.belief_history = []
        self.causal_links = {}
        self.initialize_default_beliefs()

    def update_belief(
        self, key: str, value: str, confidence: float = 0.7, decay_factor: float = 0.95
    ) -> None:
        """
        更新单个信念，记录历史。
        """
    ...
    def get_belief(self, key: str) -> Optional[Dict]:
        ...
        decayed_confidence = belief["confidence"] * (
            belief["decay_factor"] ** (hours_passed / 24)
        )
    ...
    def get_belief_summary(self) -> Dict:
        ...
        high_confidence = {
            k: v for k, v in beliefs.items() if v["current_confidence"] > 0.7
        }
        medium_confidence = {
            k: v for k, v in beliefs.items() if 0.4 <= v["current_confidence"] <= 0.7
        }
        low_confidence = {
            k: v for k, v in beliefs.items() if v["current_confidence"] < 0.4
        }
    ...
    def update_beliefs_from_news(self, news_summary: str) -> list:
        """
        根据新闻摘要自动调整信念。
        """

        all_beliefs_keys = list(self.beliefs.keys())
        print(f"[update_beliefs_from_news] 输入news_summary: {news_summary[:10]}...", file=sys.stderr)
        results = LangChainTools.parse_news_beliefs_with_langchain(news_summary, all_beliefs_keys)
        print(f"[update_beliefs_from_news] 解析出{len(results)}条news_belief", file=sys.stderr)
        for item in tqdm(results, desc="更新信念"):
            key_object = item.get("news_key_object")
            sentiment = item.get("news_sentiment", "").lower()
            confidence = float(item.get("news_confidence", 0.5))
            print(f"[update_beliefs_from_news] belief: {key_object}, sentiment: {sentiment}, confidence: {confidence}", file=sys.stderr)
            if key_object and sentiment != "":
                self.update_belief(key_object, sentiment, confidence)
        return results
